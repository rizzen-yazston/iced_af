...
use crate::{
    ...
    window::{
        about::display_about, confirm_exit::display_confirm_exit, fatal_error::display_fatal_error,
...
use iced::{
    widget::{column, text},
    window, Command, Element, Length, Point, Size,
};

#[cfg(not(feature = "i18n"))]
use iced::Alignment;

use std::{any::Any, collections::hash_map};
...
impl MainLocalisation {
    pub fn try_new(
        ...
        let title = {
            ...
            values.insert(
                ...
                PlaceholderValue::TaggedString(
                    localisation
                        .localiser()
                        .literal_with_defaults("word", "main_i")?,
                ),
            );
        ...
        #[cfg(feature = "i18n")]
        let message = localisation
            .localiser()
            .literal_with_defaults("application", "message")?;

        #[cfg(not(feature = "i18n"))]
        let message = "Just a mini application framework to create applications.".to_string();
...
impl WindowTrait for Main {
    ...
    fn view(&self, _id: &window::Id) -> Element<ApplicationMessage> {
        ...
        // Menubar
        content.push(self.menu_bar.view().map(|message: MainMenuBarMessage| {
            ApplicationMessage::Main(MainMessage::MenuBar(message))
        }));
    ...
    fn try_update_localisation(
        ...
        self.menu_bar
            .try_update_localisation(localisation, environment)?;
        Ok(())
    }
...
pub fn display_main(
    ...
    if let hash_map::Entry::Vacant(e) = application.windows.entry(WindowType::Main) {
        e.insert(Box::new(Main::try_new(
            #[cfg(feature = "i18n")]
        ...
        ApplicationMessage::Main(main_message) => match main_message {
            MainMessage::MenuBar(menubar_message) => match menubar_message {
                ...
                MainMenuBarMessage::Exit => {
                    let Some(window) = application.windows.get(&WindowType::Main) else {
                        return Ok(display_fatal_error(
                            application,
                            ApplicationError::WindowTypeNotFound(WindowType::Main),
                        ));
                    };
