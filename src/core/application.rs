...
use super::{
    environment::Environment, error::ApplicationError, session::Session, traits::AnyWindowTrait,
};
use crate::{
...
use iced::{
    ...
    widget::{column, text},
    window as window_iced, Command, Element, Subscription, Theme,
};
...
pub struct ApplicationThread {
    ...
    #[cfg(feature = "first_use")]
    first_use: bool, // Indicates if application is running for the first time.
}

impl ApplicationThread {
    fn try_new(
        flags: StartUp,
    ) -> Result<(ApplicationThread, Command<ApplicationMessage>), ApplicationError> {
        let environment = Environment::try_new(&flags)?;

        #[cfg(feature = "i18n")]
        let localisation =
            Localisation::try_new(&environment, &flags.session.settings.ui.language)?;

        #[cfg(feature = "i18n")]
        println!("Localisation initialised."); // Keep this line

        let mut window_ids = HashMap::<window_iced::Id, WindowType>::new();
        let mut windows = HashMap::<WindowType, Box<dyn AnyWindowTrait>>::new();
    ...
    fn try_update(
        ...
        match message {
                            ...
                            window_iced::Event::CloseRequested => {
                                let Some(window_type) = self.window_ids.get(&id) else {
                                    return Ok(display_fatal_error(
                                        self,
                                        ApplicationError::WindowIdNotFound(id),
                                    ));
                                };
                                ...
                                if window.is_enabled() {
                                    match window_type {
                                        WindowType::Main => {
                                            if self.is_fatal_error {
                                                ...
                                                command = display_confirm_exit(
                                                    self,
                                                    #[cfg(feature = "log")]
                                                    "Window decoration button was pressed \
                                        ...
                                        WindowType::Preferences => {
                                            command = preferences::close(self, id)?
                                        }
                                        _ => {
            ...
            ApplicationMessage::Main(_) => command = update_main(self, message)?,
            ApplicationMessage::Preferences(_) => command = update_preferences(self, message)?,
        }
        Ok(command)
    }
    ...
    pub fn close(
        ...
        let mut _parent_option: Option<WindowType> = None;
        {
            ...
            let Some(window) = self.windows.get_mut(window_type) else {
                return Ok(display_fatal_error(
                    self,
                    ApplicationError::WindowTypeNotFound(window_type.clone()),
                ));
            ...
            if _parent_option.is_none() {
                return Ok(display_fatal_error(
                    self,
                    ApplicationError::ExpectedWindowParent(window_type.clone()),
                ));
            }
        }
        let parent = _parent_option.unwrap();
        let Some(parent_window) = self.windows.get_mut(&parent) else {
            return Ok(display_fatal_error(
                self,
                ApplicationError::WindowTypeNotFound(parent),
            ));
        };
    ...
    pub fn save_and_exit(&mut self) -> Command<<ApplicationThread as Application>::Message> {
        ...
        if window.is_some() {
            // Could not be present, on first time use.
            if self.is_unsaved() {
                // flush unsaved data to files/databases
            }

            #[cfg(feature = "persistent")]
            let _ = self.session.save();
        }
        let mut commands = Vec::<Command<<ApplicationThread as Application>::Message>>::new();
        let iterator = self.window_ids.iter();
        for (id, _window) in iterator {
    ...
    pub fn spawn_with_disable(
        ...
        let Some(parent_window) = self.windows.get_mut(parent) else {
            return Ok(display_fatal_error(
                self,
                ApplicationError::WindowTypeNotFound(parent.clone()),
            ));
        };
    ...
    fn resized(
        ...
        match window_type {
            WindowType::ConfirmExit => {
                self.session.settings.ui.confirm_exit.size = (width as f32, height as f32)
            }
            WindowType::FatalError => {
                self.session.settings.ui.fatal_error.size = (width as f32, height as f32)
            }
            WindowType::Preferences => {
                self.session.settings.ui.preferences.size = (width as f32, height as f32)
            }
            WindowType::Information => {
                self.session.settings.ui.information.size = (width as f32, height as f32)
            }
            WindowType::Main => self.session.settings.ui.main.size = (width as f32, height as f32),
            WindowType::About => {
                self.session.settings.ui.about.size = (width as f32, height as f32)
            }
        }
        Ok(Command::none())
    }
    ...
    fn moved(
        ...
        match window_type {
            WindowType::ConfirmExit => {
                self.session.settings.ui.confirm_exit.position = Some((x as f32, y as f32))
            }
            WindowType::FatalError => {
                self.session.settings.ui.fatal_error.position = Some((x as f32, y as f32))
            }
            WindowType::Preferences => {
                self.session.settings.ui.preferences.position = Some((x as f32, y as f32))
            }
            WindowType::Information => {
                self.session.settings.ui.information.position = Some((x as f32, y as f32))
            }
            WindowType::Main => self.session.settings.ui.main.position = Some((x as f32, y as f32)),
            WindowType::About => {
                self.session.settings.ui.about.position = Some((x as f32, y as f32))
            }
        }
        Ok(Command::none())
    }
